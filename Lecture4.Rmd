...
\[
\text{Pr}(X_1, \dots, X_n) = \sum_{i=1}^n \text{Pr}(X_i) = p^{\sum_{i=1}^n X_i}(1-p)^{n-\sum_{i=1}^n X_i}
\]
...
+ Essentially, we pretend that the variance is $\sqrt{\hat{p}(1-\hat{p})/n}$ instead of $\sqrt{p(1-p)/n}$.
...
+ If we test the null hypothesis $\mu = \mu_o$ against $H_1: \mu < \mu_o$ using the test statistic $\bar{X}$, show the rejection region of $\bar{X} < \mu_o - 2.33\sigma/\sqrt{n}$ has significance level $\alpha = 0.01$.
    ...
    \]
    - The rejection region is defined by $\{X: \theta < \mu_o\}$
    - But the rejection region needs to be at a certain level of significance
    \[
    \text{P}_{X}(\theta < \mu_o) = \text{P}(\frac{\bar{X}-\theta}{\sigma/\sqrt{n}} > \frac{\bar{X}-\mu_o}{\sigma/\sqrt{n}}) = 0.01
    \]
    - Again, since $\frac{\bar{X}-\theta}{\sigma/\sqrt{n}} \sim \mathcal{N}(0, 1)$
...
+ Clearly, for a symmetric sampling distribution, the rejection region for a two-sided test is twice the size of that for a one-sided test given that $\alpha$ and *the null hypothesis* are the **same**.
...
In the cases above, we can test the null hypotheses $\mu = \mu_o$ or $p = p_o$ by using the sampling distribution of $Z$, $T$, and $Y$.
...
+ What is the random sample?
    ...
    - The external temperature at his house $(X_1, \dots, X_56)$
